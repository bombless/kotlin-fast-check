...
export async function findGradleProjectRoot(file: string): Promise<string | undefined> {
  const end = perf.start("ProjectDiscovery.root", { sourceFile: file });
  ...
  while (true) {
    ...
    if (hasSettings || hasWrapper) {
      end({ result: "found", projectRoot: current });
      return current;
    }
    if (hasBuild && nearestBuildRoot === undefined) nearestBuildRoot = current;
    const parent = path.dirname(current);
    if (parent === current) {
      end({ result: nearestBuildRoot ? "found-build" : "not-found", projectRoot: nearestBuildRoot });
      return nearestBuildRoot;
    }
    current = parent;
  }
}
...
export async function discoverCompileSdk(projectRoot: string): Promise<number | undefined> {
  const end = perf.start("AndroidSDK.compileSdkDiscovery", { projectRoot });
  ...
  for (const root of roots) {
    for (const name of candidates) {
      ...
      if (match) {
        const compileSdk = Number(match[1]);
        end({ compileSdk });
        return compileSdk;
      }
    }
